...
# Load Dataset
```{r}
PATH <- "D:/OneDrive - Bina Nusantara/4th Semester/Data Mining and Visualization - DTSC6005001/healthcare-dataset-stroke-data.csv"
df <- read.csv(PATH)
```
...
```{r}
rf_model1 <- randomForest(stroke ~ ., data = strokeTrain)
importance(rf_model1, type = 2)
```
In the first model, this random forest uses all variables as predictor variables and stroke as the target. which will be compared with the second model where the second model does not use all variables.
...
### Fit the Random forest classifier with three important variables
```{r}
rf_model2 <- randomForest(stroke ~ age + bmi + avg_glucose_level, data = strokeTrain)
importance(rf_model2, type = 2)
```
```{r}
varImpPlot(rf_model2)
```
In this second random forest model, only the three most important variables mentioned from the first random forest model summary are used. 

### Model validation of random forest classifier
```{r}
predictionRF <- predict(rf_model1, newdata = strokeValidation)
predictionRF2 <- predict(rf_model2, newdata = strokeValidation)
```
...
sprintf("Sensitifity Scores is %s", sen1)
cm
```
```{r}
cm <- table(predictionRF2, strokeValidation$stroke)
accuracy <- round(sum(diag(cm)) / sum(cm), 3)
sen2 <- Sensitivity(strokeValidation$stroke, predictionRF2)
sprintf("Accuracy is %s", accuracy)
sprintf("Sensitifity Scores is %s", sen2)
cm
```
### Conclusion of random forest model
In the first model, random forest uses all variables in determining classification and produces an accuracy of 94.4% and a sensitivity value of 99.8%, while the second random forest model only uses three dependent variables for the model in determining classification and produces an accuracy of 94.2% and a sensitivity value of 99.6%, which means the model has a high level of ability to detect positive conditions or diseases in truly positive patients. 

However, in choosing a model, there are several factors to consider. Firstly, the difference in accuracy between the two models is 0.002, which is a very small difference. However, if accuracy is the main determining factor, then the first model of random forest with higher accuracy may be a better choice.

Furthermore, if we consider the interpretability of the model, by using the significant variables in the second model of the random forest model, we can understand the important factors that have a significant influence on the prediction. Thus, the model can provide a clearer and more focused explanation of the variables that are important in making predictions.

Therefore, the model chosen is the second model of random forest because the accuracy has only a small difference and the important variables used can explain the factors that have a significant effect on prediction.

# Decision Tree
### Fit the Decision tree classifier with all variables
```{r}
DTModel1 <- rpart(stroke~., data= strokeTrain, cp= 0.001, method= "class")
...
### Fit the Decision tree classifier with three most important variables
```{r}
DTModel2 <- rpart(stroke~age+avg_glucose_level+bmi, data= strokeTrain, cp= 0.001, method= "class")
DTModel2$variable.importance
```

```{r}
rpart.plot(DTModel2, type = 2)
```

### Model validation of decision tree
```{r}
DTPred1 <- predict(DTModel1, newdata = strokeValidation, type = "class")
DTPred2 <- predict(DTModel2, newdata = strokeValidation, type = "class")
```

### Model performance of Decision tree models
```{r}
cm <- table(DTPred1, strokeValidation$stroke)
accuracy <- round(sum(diag(cm)) / sum(cm), 3)
sensitifity <- Sensitivity(strokeValidation$stroke, DTPred1)
sprintf("DTModel1 Accuracy is %s", accuracy)
sprintf("DTModel1 Sensitifity Scores is %s", sensitifity)
cm
```

```{r}
cm <- table(DTPred2, strokeValidation$stroke)
accuracy <- round(sum(diag(cm)) / sum(cm), 3)
sensitifity <- Sensitivity(strokeValidation$stroke, DTPred2)
sprintf("DTModel2 Accuracy is %s", accuracy)
sprintf("DTModel2 Sensitifity Scores is %s", sensitifity)
cm
```
### Conclusion of decision tree model
In the first model, decision tree uses all variables in determining classification and produces an accuracy of 93.2% and a sensitivity value of 98%, while the second random forest model only uses three important dependent variables for the model in determining classification and produces an accuracy of 94.4% and a sensitivity value of 99.5%, which means the model has a high level of ability to detect positive conditions or diseases in truly positive patients. 

Because the second model of the decision tree has a high accuracy and sensitivity level in predicting the True Positive value, the second model of the decision tree is the best model compared to the first model.

# Final Conclusion
After conducting experiments using two models, namely random forest and decision tree, the results show that the random forest model tends to provide better accuracy values with the addition of more features. However, at the same time, the decision tree model has a tendency to perform worse when given more features.

Based on the model performance between the second random forest model compared to the second decision tree model, the second decision tree model that uses only three important variables has better values both in terms of accuracy and sensitivity.

For the final model, we used second decision tree model with only three important variables: age, avg_glucose_level and bmi. For the evaluation of this model, we used a confusion matrix.

- True Positive (TP): The number of data correctly predicted as "having stroke" by the second decision tree model is 3.
- False Negative (FN): The number of data that should have been classified as "not have stroke" but was incorrectly predicted as "have stroke" by the second decision tree model was 4.
- False Positive (FP): The number of data that should be classified as "have stroke" but wrongly predicted as "not have stroke" by the second decision tree model is 53.
- True Negative (TN): The number of data that were correctly predicted as "do not have stroke" by the second decision tree model was 962.

The overall accuracy of this second decision tree model is 95% with 99% sensitivity, which means it can accurately predict TP (True Positive) with a very minimum error rate. True positives are very important for medical data as TP can lead people to live a much healthier life even if the prediction is wrong.
